# -*- coding: utf-8 -*-
"""
Created on Thu Apr  1 19:06:34 2021
...
params = {}
#依次为读取文件路径和保存文件路径
params['path'] = '/usr/local/data/lab_shouce/small_testdata_2_N15_M07_F04.csv'
params['opath'] = '/usr/local/data/lab_shouce/testfea_2_demo.csv'
...
try:
    ...
    def feature_get(filepath):
        ...
        feature_list = [i for i in range(len(data))]
...
#            #包络的R值和J值
#            h = fftpack.hilbert(df_line)
#            z = np.sqrt(df_line**2 + h**2)  #包络
#            m2 = np.mean(z**2) #包络二阶矩
#            m4 = np.mean(z**4) #包络四阶矩
#            Ps = np.mean(df_line**2) / 2
#            envelope_r = np.abs((m4-m2**2) / (m2**2))
#            envelope_j = np.abs((m4-2*m2**2) / (4*Ps**2))
